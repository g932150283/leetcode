...
public:
    static void knapsack(std::vector<int>& selectedWeights, std::vector<int>& selectableWeight) {
        int sumOfWeights = std::accumulate(selectedWeights.begin(), selectedWeights.end(), 0);
        ...
        if (sumOfWeights == KNAPSACK_MAX_WEIGHT) {
            ...
            sumOfWeights = std::accumulate(selectedWeights.begin(), selectedWeights.end(), 0);
            RESULT = std::max(RESULT, sumOfWeights);
            return;
        } else {
            RESULT = std::max(RESULT, sumOfWeights);
        ...
        for (int i = 0; i < selectableWeight.size(); i++) {
            ...
            if (std::find(selectedWeights.begin(), selectedWeights.end(), num) != selectedWeights.end()) {
                continue;
            }
...
int main() {
    ...
    Solution::knapsack(selectedNums, Solution::WEIGHTS);
